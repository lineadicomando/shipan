...
### The second imperial print, which lodges the centre and names both readings

《御定奇門寶鑑》, 六卷 — 故宮珍本叢刊 **第431冊 陰陽五行**, 第一冊（共二冊）, 故宮
博物院編, 海南出版社 2000. It was the largest unopened dunjia work left on the
shelf and it was left last for a reason that turns out to be a misreading of its
own record page: the shelf had it as «one 冊 of eight». **It is one of two, and
the half held is the whole of the prose.** Its 新編目錄, at series p. 368 where
the publisher says it puts them, divides the work 第一冊 = 卷一至卷六 附陰遁一局至
陰遁六局 and 第二冊 = 附陰遁七局至陽遁九局. All six juan are here; what is
elsewhere is twelve of the eighteen tables. The «全8冊» that misled is the price
line — 定價1290元（陰陽五行18種共8冊）— and describes the subject group of the
reprint series, not this work.

**卷二 opens on 起例, and its first entry states the centre's lodging for both
dun and then names the other reading.**

> 坎一天蓬。坤二天芮。震三天冲。巽四天輔。中五天禽。乾六天心。兌七天柱。艮八
> 天任。離九天英。**陽遁陰遁俱寄坤宮。一本陰遁寄艮。**

Series p. 40, read at 600 dpi. The first sentence is the plainest statement of
`centreLodging: kun` this shelf holds: no condition, both dun, in the enumeration
that assigns every star its palace. **The second is the first attestation
anywhere here of the value the parameter refuses.** `docs/parameters.md`'s 寄宮
`dun` — the centre lodging at 艮 in a yin board — was declared and refused with
the register saying, in as many words, that it is what nobody states. Somebody
states it.

**What it does not do is implement it, and the reason is a rule in
`CLAUDE.md`.** 「一本」 is the language of textual criticism: *one copy reads*. A
disagreement between witnesses to one text is not a divergence between
practitioners, no value is ever named for a recension, and an edition can only
be preferred. So what this line establishes is narrower than it first looks and
still worth the reading: the 艮 reading exists in transmission, the compiler of
an imperial edition knew it, and he printed 坤 for both dun and put the other in
a note. Two witnesses agreeing, or one text checking itself, is what would move
the value; a flagged variant is neither.

**And the same juan lodges the centre again where it matters — inside the
laying.** 「以旬首取符使法」 walks all six decades of 陽遁一局 and gives the
centre's this way:

> 甲辰在中宮，寄於坤二，天禽為本旬直符，死門為本旬直使。管至癸丑十時，又換
> 符使。

Series p. 42, read at 600 dpi. 在中宮 and 寄於坤二 in one clause, and then the
decade is worked from 坤: the 直符 and the 直使 are **lodged out** of the fifth
palace, not stood in it. **That is the counter-witness the palace-5 divergence
did not have.** 《奇門遁甲金鏡寶鑑》 tabulates the opposite for every hour of
every 局 — see above — and both are Qing imperial woodblocks in the same series.
A quantity with a print on each side is a divergence between practitioners in the
form `CLAUDE.md` describes, and this engine's behaviour is the one this print
states.

**Its 八神 are a fifth convention, and they pair the seats the way this engine
does.**

> 一直符。二螣蛇。三太陰。四六合。五勾陳。六朱雀。七九地。八九天。此陽遁也。
> 陰遁則一直符。二九天。三九地。四朱雀。五勾陳。六六合。七太陰。八螣蛇。
> **朱雀下有元武。勾陳下有白虎。**

Series p. 41, read at 600 dpi, the last sentence set as an interlinear note. The
eight names are the same in both dun; what the yin dun reverses is the **order
they are counted in**, which lands them on the seats this engine's
counterclockwise ring lands them on. And the note pairs 白虎 with 勾陳 and 元武
with 朱雀 — 元武 for 玄武, the reign taboo again, on the same shelf and in the
same decade. **Three prints now give this engine's pairing of the middle two
seats** — this one, 《金鏡寶鑑》 and, inverted, 《全局》 — and none of them
agrees with another about what makes the alternate name apply. That is the shape
of the missing parameter, stated once more: not which eight names, but which fact
selects the pair.

**And it closes an open question the last session left with nowhere to arrive.**
《金鏡寶鑑》's 卷之五 三元訣 has a cut 坤 where the parallel wants 孟, and on the
only reading that parses it gives the hour system the 孟-first anchor that
遁甲演義 assigns to its 月家. That was written down so a second witness would have
an address. This is it, in prose and worked:

> 定三元法。三元者，上中下三局也。一元五日。以甲己二干為一元之首，謂之符頭。
> 符頭所臨之支，如值子午卯酉，則為上元；值寅申巳亥，為中元；值辰戌丑未，為
> 下元。蓋甲子日起上元，越五日己巳起中元，又越五日甲戌起下元，三局已周。

Series p. 41, read at 600 dpi. 仲 first, which is `ju.ts` and 遁甲演義's 時家 in
prose, now in a second Qing imperial print with its own five-day count. The
金鏡寶鑑 leaf stands alone against three statements and two implementations, and
the question it opened is answered rather than merely unmoved.

**What else the two juan read confirm.** The 六儀 assignments 甲子遁於六戊 through
甲寅遁於六癸; 分順逆法 「冬至後用陽遁，夏至後用陰遁」; the 三元定局 as a verse for
both dun, 「冬至驚蟄一七四，小寒二八五為期」 and its yin twin; 直符加時干法 and
直使加時支法, which are 值符隨時干 and 值使隨時支 in the engine's words — 「若用
時之干臨於地盤何宮，即以天盤直符加於此宮」. None of it moves a rung on its own
and all of it is the ordinary business of a second witness.

**What has not been read.** 卷一 was read for its 奇門源流 and its 凡例十則 —
which are bibliography and 選擇 practice, not procedure — and 卷二 for the 起例 as
...
《遁甲集成》第六冊's 統宗 prints it, per the note below. 三奇得使 is not its
precedent: there the sources disagree,
...
a stem over a stem. At its pp. 810–811 it gives 驚加乾用法, 驚加坎用法, 驚加艮
用法, each with a 剋應 under it: 「主老少不和，宅舍生非，陰人為禍…在物主空鳴
鍾磬之聲，或金銀人物、鴛鴦鳥獸之物」. That is an omen told to a querent, which
is the register's own example of what does not get imported, and it is not the
table above under a variant name. Written down because the word is the search
term, and eight sampled sheets of that half returned it and nothing else.
Surveyed and one leaf read on the plate 2026-08-28.
...
年家奇門定局, 月家奇門 tables, and a stated 起日奇 rule with 日家 tables under
it. Its 年家 is the same object as 遁甲演義's and not a selection table: one
board to an 元, each 元 running sixty years — 「起上元年家奇門，陰遁一局，管上元
六十年，總在此輪」 — cast in 陰遁, with the 直符 and the 直使 given per year. And
it assigns the three 元 differently:
...
遁甲演義 states it 「三元年遁：上元六十年以陰遁一局加之，中元六十年以陰遁四局
加之，下元六十年以陰遁七局加之」. The 集成 announces its own in the heading of
the first table — 「上元起年家奇門奇儀星門所在之宮定位，**以一七四為法**」 — and
then prints the three, a leaf of tables to each: 上元一局, 「中元年家奇門定局…
中元七局」, 「下元年家奇門定局…下元四局」. It dates itself 「今康熙二十三年交上元
...
which is what makes this a disagreement rather than two unrelated tables: the
same three numbers over the same three 元, in a different order.

**What that does to the parameter.** `system: nianjia` was refused for want of
a second witness. It has one, and the witness contradicts, so the value stays
refused — for a stated reason and no longer for want of a text. The standard
this file sets is two witnesses agreeing or one text that checks itself; two
witnesses disagreeing meets neither, and preferring the older or the
better-printed would be this engine choosing a school in silence. **The errand
this phase most wanted is finished, and it finished as a negative**, which is
a documented outcome and not a lesser one.

**Whose the tables are is not established, and it does not have to be.** They
stand under no title leaf this survey found: 《遁甲符應經》 opens at the volume's
p. 1136 and its 天象分野圖 closes at p. 1159, and p. 1160 begins the 年家 with no
break but a change of subject — while the 康熙二十三年 here and the 康熙二十七年
in the 月家 tables cannot be a Song work's own. The reproduction's 版心 carries
no printed running title, only modern hand numbering, so the running-title
method that settled two sections of 第六冊 cannot be run on this volume at all.
...
**And the volume itself is now in question, which is one level further out.**
第一冊 carries a typeset 總目錄 for all six volumes, surveyed 2026-08-31, and it
gives 《遁甲符應經》三卷 to 第一冊 at series p. 385 and 第三冊 to a work it names
...
is one the series' own contents list attaches elsewhere. **It changes nothing
about what those leaves say** — the clause and the tables were read on the plate
and are unaffected — and it changes what they may be cited as. Settling it is a
collation of 第三冊 pp. 1136–1159 against 第一冊 pp. 385–490, and the 目錄 above
says which section to open.
...
conversion, and nobody has said as of when».

**A primary has since answered the third objection and unsettled the first.**
《授時曆故》, 黃宗羲, carries the complete 黃道宿次 for all twenty-eight lodges
in its 卷二 — the eastern seven summed at 七十八度一十二分, the northern at
九十四度一十分七十五秒, the western at 八十三度九十五分, the southern at
一百〇九度八分 — and beside it prints how the table is made:
「**授時黃道宿度依赤道及冬至歲差所在算定，若上下考驗歲差每移一度，依術推變，
各得當時宿度。凡上下消長皆從虛度。**」 The worked step under it is anchored —
「元辛巳歲前冬至後赤道距後度四十分…如赤道率一度〇八六五而一」 — and the
work's own 提要 says what that means: 「授時術不用積年日法，憑本年實測所得氣閏
諸應，上考下求，**即以至元辛巳為元**」.

Three things follow and they do not all point the same way. **The epoch
objection is answered**: 至元辛巳, 1281, stated by the work and not inferred
from it. **The conversion objection is confirmed from the primary side**,
which is worth more than 朱文鑫 saying it — 依赤道…算定 is the text's own
account of its method. **And the inheritance claim sits awkwardly**: 朱文鑫
writes 「元授時黃道度與宋紀元同」, where 授時曆故 presents the table as computed
from the 赤道 degrees and the solstitial 歲差 rather than taken over. A primary
describing a derivation and a secondary describing an inheritance are not
flatly contradictory — a derivation can reproduce what it inherits — but the
register should not go on citing only the second.
...
The rate arrives at twenty-eight years three ways, and they are one motion:
...
| 一日行三分五十七秒, the 大數 procedure below — 《張果星宗》, whose 三辰通載 section opens it | 12.857 °/yr | 28.00 yr |

**A fourth transmission says twenty-nine, and it is not a misprint.** Three
witnesses print a 星曜行度 table of round numbers in which 「紫氣二十九日行一
度，大約二十九月一宮，二十九年一周天」: 《古今圖書集成》 藝術典 星命部 卷565–577
(read on the plate, 第468冊 leaf 23), 《星平集腋統宗》 at 故宮 423 p. 164 (道光壬辰
1832), and 楊國正's 七政四餘論命術講義 p. 12, which reprints the 集成's wording
including its closing clause. The three columns of each row check against each
other — 29 × 365.25 = 10 592 days = 29.00 years — so one digit did not slip.
**And the same work carries both numbers.** The 紫炁總論 that stands two columns
from the 大數 procedure on the same leaf ends 「凡二十九年行一周」. Read on the
plate 2026-09-01. What that settles is written below, under the layers.
...
**It is stated as a procedure, with an epoch constant, and this entry used to
say two independent lines carried it. That was one line counted twice.** The
claim was that 《古今圖書集成》 星命部 carried a 定度法 for each of the eleven
under the heading 三辰通載·紫氣, and that 《張果星宗》 卷八 氣星篇 carried the
same one. The 集成's own 卷目錄, read on the plate, says what the 集成 is
reproducing:

> 第五百八十卷目錄　星命部彙考十六　張果星宗十四　　太陽算法 日篇 …
> 炁星篇　紫氣總論　紫氣躔度　紫氣歌斷　紫氣入宮　算法 孛星篇 …

The eleven 算法 are 《張果星宗》, printed inside the 星命部彙考 — 張果星宗 十二
through 十五 at 卷578 to 卷581 — and **三辰通載 is not a second work but the
opening section of 張果星宗十二** at 卷578, which is why the vernacular
abridgement files the procedure under that name. So the 集成 and 《張果星宗》 are
one witness reproduced twice, not two agreeing witnesses, and this file's
standard is not met by them. Read on the plate 2026-09-01, 第469冊 leaves 1, 19
and 24 for the three 卷目錄 and leaf 24 for the procedure itself:

> 紫氣筭法　炁星篇
> 置積日減一千二百八十八。以一萬二百二十八大數除之。不盡爲殘分。轉一當十。以
> 二百八十爲一度。二十八爲一分。次下除爲秒。平行：一日行三分五十七秒。

**What the plate does buy is the text.** The procedure was known here only from
`gujin-tushu-jicheng-xingming-baihua` p. 234, a modern vernacular abridgement
photographed with a phone; it is now read at 300 dpi off the 中華書局 影印, and
every character of it, the 大數 10228 and the offset 1288 included, stands as
quoted. That is a collation and it moves the text, not the count — see § "What a
second copy of one text buys".
...
same series gives 羅睺 「置積日加五百六十，以六千七百九十四逆遊數除之」 at 卷581
and 月孛 「置積日加一千二百三十五，以三千二百二十五大數除之」 on the 紫氣's own
leaf, 計都 being 「將羅睺定度加半周天數」 and so not independent. Both read on the
plate 2026-09-01, 第469冊 leaves 24 and 30. Against the sky:
...
**The tradition says 紫氣 has no referent, in its own words, and says the same
of 月孛.** The 紫炁總論 that follows the procedure on the same leaf opens 「天乙
紫炁。續木之餘。**在天無象**。以木德最厚。立此名以追步。其餘光用之。效久不
廢。其行度無伏見遲留」 — in the heavens it has no figure; the name was instituted
in order to track it; its residual light is used, and the effect has long endured
undiminished; and its motion has no station and no retrogradation, which is the
uniform prograde motion the procedure computes. The 月孛總論 on the same leaf
opens the same way — 「太乙月孛乃水之餘。**在天無象**」 — for a body this engine
does place, from `SE_MEAN_APOG`. **So «no referent» is this project's
distinction and not the source's**, and stating it as the tradition's would be
attributing to it a division it does not make. What is true is narrower: the
apogee has a modern referent and 紫氣 has none, which is a fact about what
astronomy recovered and not about how the four were transmitted.

**The twenty-eight and the twenty-nine are layers of one text, which is now
shown rather than inferred.** They stand two columns apart on one leaf, in one
work, under one compiler: a 算法 whose 大數 is a computational constant, and a
總論 whose 「凡二十九年行一周」 belongs with the 星曜行度 table of round numbers,
where 月孛 is «nine» for 8.85 and the nodes «eighteen» for 18.6. **The round
layer is gradeable against the sky on the two bodies that have a referent, and
it fails**: propagated from 1886 to 2026 the mnemonic 月孛 is 98° out and the
mnemonic nodes 87°, against 11.4° and 0.2° for the 大數. A layer that misses by
three palaces on both checkable bodies is a table for remembering and not for
computing, and this engine already declines it for the three it places. That is
a divergence recorded, not a school declared: a layer is not a lineage, and
`docs/parameters.md`'s rule that no value is ever named for a recension holds
...
referent in the sky, which is the one thing 紫氣 does not have.**

**That much stands. What this entry used to draw from it does not, and the
arithmetic is the reason.** It read «unbounded in principle, and no source can
close it», which describes a quantity; what the table above measures is a
quantity times a lever arm. The drift a rate error produces is ε × 360° × the
circuits run since the anchor, so 0.20 % reaches 69° over the 726 years from
1300 and 3.6° over the 4.97 circuits from 1886. **A rate error is bounded and an
anchor's resolution is bounded, so their sum is**; what is unbounded is only the
distance a calibration is carried. Both bounds are readings of the evidence and
neither is a property of 紫氣. The 1886 board below is where an anchor would come
from, and what it is worth is a palace: the error on a placement taken from it
is that palace's ±15° plus about 4° of drift if 紫氣's constant is no worse than
the worse of its two checkable siblings — and nothing establishes that it is not
...
withheld: 年命, the 年神 bearings and 太乙's 大遊 ship at the same grade.
...
test any solved origin has to pass. **It is also an epoch, in the sense the
arithmetic needs**: a linear rule is fixed by a rate and any one dated position,
and the 積日 origin the printed procedure asks for is redundant once a dated
position is in hand. What was missing was never a citable epoch. What is missing
is a second one.

**Where the two quantities stand on the ladder, and it is the same rung.** The
**rate** is one work — 《張果星宗》, in two impressions of itself — which
contradicts itself between its 算法 and its 總論 and is graded only through its
two siblings; a text that disagrees with itself about the quantity is not a text
that checks itself, so it is rung 5 and not rung 4. The **anchor** is one plate,
《星度指南》 1886, redundant about the chart and not about 紫氣: rung 4 can be
argued and rung 5 can, and `docs/notes.md` says the register takes the weaker.
Both at 5. **That is a fall**: this entry stood on «two witnesses, which is the
count this file asks for», and the count was wrong. Nothing was overturned by a
better source — the shelf grew and the reading of what was already on it
corrected.

**And the 講義 is a third impression rather than a third witness.** 楊國正's
七政四餘論命術講義 p. 42 prints the 紫炁總論 word for word, including 「在天無
象」 and the 廟旺 that follow, and its p. 12 prints the 星曜行度 table with the
集成's own closing clause; where the 1832 《星平集腋統宗》 parts from both at the
nodes — 廿日 against 十八日 — the 講義 sides with the 集成. Its 119 pages carry
**no worked chart**: 命例, 排盤, 年生 and 民國 return nothing on its text layer,
which locates no second anchor there and, the layer being an aid rather than a
plate, closes nothing.
...
`epoch` is declared with `jinjing` alone, and **the other lineage's figure is
now known and congruent**: 《統宗》 counts from a 公元基數 of 10 153 917, which
differs from 《金鏡》's by an exact multiple of both 360 and 24 and therefore
lays this board identically. See § "The witness that is not a manual" below,
where the arithmetic is done. So the parameter keeps one implemented value for
the reason it was always going to keep one if the reading went this way — not
because a branch is unread. What is still unread is the 上元積年 as
《太乙統宗寶鑑》 itself states it: 錢大昕 discusses that work's 積年術 in
《十駕齋養新錄》 卷十四, and whether 寶鑑 and the 統宗大全 on this shelf are one
recension is not settled here.
...
five numerals — and **a second witness is now partly on the shelf without
lifting any of them**. 《太乙數統宗大全》 has been opened on four leaves: its
卷二 is the calendrical apparatus, giving 日法 一萬單五百 with 朔實, 朔策 and
朔盈差 beside it, and its 卷三 carries 求月計 · 日計 · 時計太乙入紀元局術 among
the 起例. That is another system's constants, not a collation of these: it
would let the three registers be computed **on 《統宗》's arithmetic**, which
is a different board from the one this engine lays, and it says nothing about
what 卷一 of 《金鏡》 meant to print. Until a witness carrying *these*
constants is read, `yueji`, `riji` and `shiji` stay declared, refused by name,
and this is the entry saying why.

**That negative was closed against the shelf of 2026-08-30, a 太乙 volume
arrived the next day, and the question has since been put to it.**
《太乙數統宗大全》, 故宮珍本叢刊 第420冊 — four hundred and forty pages, and the
first 太乙 witness here besides the 金鏡式經. Whether it carries 卷一's constants
was the question, and the answer is the paragraph above: it carries constants of
its own, on another calendar, and none of 《金鏡》's. Four of its four hundred
and forty leaves have been read, which is what that answer rests on. Last
checked 2026-09-03.
...
**參將 is the one step *this* text never states in words.** It is read off the
text's own worked boards, which put it a quarter turn clockwise from the 大將
without exception — and 《統宗》 does state it, as 大將宮數 × 3 mod 10, which is
that quarter turn on every seat of the ring. The induction below is what the
engine was built on and what the second witness then transmitted:
...
巽→坤, 離→兌, 坤→乾, 兌→坎 — and every one of them is ×3 mod 10.

**Where the induction and the transmitted rule parted was the centre, and the
stated rule is followed there.** A count ending in five seats the 大將 in 五宮,
which stands on no ring, and no worked board of 《金鏡》 reaches the case; ×3
mod 10 gives five again, and both boards 盧央 works that reach it say so —
「主大小將均入中宮，是為杜塞」, 「客算得二十五，所以客大小將杜塞」. So the
engine seats the 參將 beside its 大將 in the middle, where it used to hand over
`undefined`: a rule that reproduces all fourteen induced instances and answers
the one case they never show is worth more than a silence, and the silence had
no attribution — no text says a 參將 stops existing. It is not a rare cell
either, a general falling there in a little under a third of years.

**What the second witness also does is name the standing, and that part does
not travel.** 杜塞 is the two generals shut into the one palace neither 太乙 nor
either eye can enter — 「中五與外面不通」 — and it is not in `patterns` with the
other seven, because those each carry the sentence 卷三 uses to say what they
*are* and this one's would be a modern scholar's; 《統宗》's own 序 lists
關囚掩迫對擊 and 挺挾 without it. The name is here, where a reader weighing the
board can find it. **And nothing at the centre enters a condition**, which is
what the ring of eight already said about a 大將 there: 太乙 never stands in the
middle, so nothing there is at a distance from it, and two bodies meeting there
are 杜塞 rather than 關.
...
text, which is unusually well equipped to be one. **What follows is that
reference; the section after it is the evidence that does not come from the
tradition's manuals at all**, and the two are kept apart because they answer
different questions. 卷三 prints a **立成 of
...
### The witness that is not a manual, and the first checks from outside the text

**《中國古代星占學》, 盧央, 中國科學技術出版社 2007, 第五章 太乙式**, 809 pages,
with a row in `docs/provenance.tsv`. It is a modern historian of astronomy
laying the board out from both surviving lineages, and it is a different kind
of source from everything else in this section: not a manual teaching the
method, but a study of it that works six dated boards through every step,
quotes a dynastic annal against them, and says of its own object 「數占是一種
關於式占的設計，並不是可信的」 — the 數占 explains what has already happened
and does not predict. A source that exposes the mechanism entire *and* declares
what it thinks of it is worth quoting on both.

**What it brings is the thing this board did not have: evidence written by
nobody teaching the method.** All of it is in `test/taiyi.test.ts`, in the
block named for it.

- **《齊書·武帝紀》 records eleven dated positions of 太乙**, from 漢高五年
  (202 BCE) to 宋升明元年 (477 CE), and 蕭子顯 was not writing a manual. 盧央
  quotes the passage whole to adjudicate the three 上元積年 in circulation: the
  count of 《太乙淘金歌》 is out by a whole 元 and gives 九宮 where the annal
  says 四宮, 「故不能採納」. **This engine reproduces all eleven**, and the 太歲
  it prints for each agrees with the reign year that names it, which is what
  shows the years were converted rightly rather than the palaces guessed.
- **Six boards worked step by step**, of which five recompute in every printed
  value: 589 through all eight steps of the 布式 with the finished figure at
  圖 5-7, then 940, 830, 124 and 210 BCE, each worked to name a configuration.
  The sixth, 207 BCE, prints a 主算 of 二十三 where the procedure of its own
  chapter gives sixteen — which is what this engine gives — and the page works
  no arithmetic for it. It is carried as an erratum for the reason the 立成's
  fourteen are.
- **The 參將 is stated in words**: 「主大將宮數乘以 3，再以 10 除之，所得餘數
  即為主參將所在宮」. ×3 mod 10 *is* the quarter turn clockwise that fourteen
  worked boards of 卷一, 卷六 and 卷九 induced, on all eight seats of the ring.
  The step this file called untransmitted is transmitted — and it answers at
  the centre, which the fourteen never reach and this engine used to leave
  empty. See § "主算 and 客算" above, where that is decided.
- **五福 against three acts of state.** 周琮's memorial of 1071, in 《宋史·禮志
  六》: the court built the eastern 太乙宮 when the body entered 巽 in 984 and
  the western when it entered 坤 in 1029, and he asked for a third in the
  capital because it would enter the centre in 1074. It was built. The
  forty-five years this engine counts with put the body in each of the three on
  the year the archive names.
- **三基 against three dates**, which is what 卷五's 甲寅 count had none of:
  君基 into 午邦 in 714, 臣基 into 午邦 in 642, 民基 into 未邦 in 627 — and for
  the first two 盧央 runs 《統宗》's own arrangement beside 《金鏡》's, where a
  start from 午邦 and a correction of 邦盈差 二百五十 compensate each other and
  land on the same fief. Two lineages, one placement.

**And the other 上元 turns out to be the one already weighed here.** 《統宗》's
公元基數 is **10 153 917**, so that 940 CE is 歲積 10 154 857 — the figure the
940 board is worked from — and 《金鏡》's is 1 936 557. The difference is
8 217 360, which is 22 826 × 360 and 342 390 × 24: the same difference § "A
second 上元" below computes for a teaching handout that named no text, which is
therefore counting from 《統宗》. `epoch` still has one implemented value, and
now because the other one agrees rather than because nobody had read it.

**Where the two lineages part is the reading, and three partings touch what
this engine reports.** None of them is an error found here; each is a second
tradition saying something 卷三 does not.

| | 《金鏡》, which this engine follows | 《統宗》, per 盧央 |
|---|---|---|
| 格 on the host's generals | not a condition: 「客目大小將與太乙對宫為格」 names one party | the 124 board marks 「主大將居九宮，又與太乙相格」 |
| 文昌 in 太乙's palace | 囚, per 「文昌將并主客大小四將，俱與太乙同宫」 | 關, on 樂產's authority — and 掩 two lines away, which 盧央 reports without choosing |
| 格 by opposition | the palace facing 太乙 across the board | the 940 board admits a 沖 of branches: 始擊 in 辰 against a 太乙 in 乾, 「乾宮居戌亥」 |

**What it does not do is settle a numeral.** It is not a witness to 《金鏡》's
text — it quotes the same 四庫 lineage — so the five broken constants of 卷一
stay broken and the three registers stay refused. What it does say about them
is the route: 「只要將歲積算改成月積算或日積算」, the 月計 and 日計 being the
same placements over a different counter, and the 時計 adding the 陰陽二局
(冬至 to 夏至 順行, 夏至 to 冬至 逆行) that `taiyiJu`'s yin branch already
computes, plus the 三元 of a day read off its 干支 exactly as 奇門 reads it.
《統宗》 has calendrical constants of its own for those counters — 日法 一萬單
五百, and a 歲實 錢大昕 showed to be the 授時曆's in disguise, which dates that
apparatus after 1281 whatever it claims of itself.

**And it reaches this project through a transcribed corpus rather than off the
plate**, which is the ordinary limit `docs/notes.md` states: passages filed by
the question they answer say where to look, and the primary annals quoted in
them — 《齊書》, 《宋史》 — are not on this shelf. What holds them up here is
that eleven positions from an annal and six boards from a study reproduce
against a procedure that knows nothing about either. Read 2026-09-03.
...
**五福太乙 — forty-five years to a palace, and two checks, one of them from an
archive.** 卷五 推五福太乙法 runs it on the 一萬三千三百三十一 count, over the
four corners and the centre, 二百二十五年 to the circuit. The text's own check
is a date: 「今開元十二年甲子在遼東十一年也」, the eleventh year in 黃始宮, which
is what the count gives. The second is not the text's — 周琮's memorial of 1071
in 《宋史·禮志六》 dates the body into 巽 in 984 and into 坤 in 1029 by the
temples the Song court built for it, and asks for a third because it enters the
centre in 1074. The count puts it in each of the three on the year named.
...
**三基 is computed, and checked from outside since 2026-09-03.** 盧央 puts all
three on dated years — 君基 into 午邦 in 714, 臣基 into 午邦 in 642, 民基 into
未邦 in 627 — and runs 《統宗》's arrangement beside 《金鏡》's for the first
two, where a start from 午邦 and a 邦盈差 of 二百五十 compensate and land on the
same fief. This engine gives all three. What follows is the count itself, which
《金鏡》 states and never checks: 君基, 臣基 and 民基 run on 卷五's opening
...
**The text it does not name is 《統宗》**: its base is that lineage's 公元基數
10 153 917 to the year, which § "The witness that is not a manual" quotes from
a source that does name it. The table:
...
waits for them. **What changes with them is the counter and almost nothing
else**, which 盧央 states outright — 「只要將歲積算改成月積算或日積算」 — with
the 時計 alone adding the 陰陽二局 that `taiyiJu` already computes and the 三元
of a day read off its 干支. The technical name for the four is **四計**, and
the first of them is **歲計** in both witnesses — which is the name the
parameter now carries, its identifier staying `nianji` so that a shared link
cannot break.
...
the first only: its 卷二 gives counts, and they are 《統宗》's own rather than a
second look at these. The 闕 is still unread against it. Last checked
2026-09-03.
...
They are four of the **十神**, the group 三基 and 五福 also belong to, and the
group has a 上元 of its own: 甲寅 rather than 甲子, the 二十八萬五千一十一 this
engine already counts the 三基 from, and 盧央 states the prohibition that count
carries — 「這個數據不能用來推求太乙行九宮和文昌行十六神」.

**十精太乙, and it is the one layer here that is not dynastic.** 天皇, 帝符,
天時, 太尊, 飛鳥, 五行, 八風, 五風, 三風 and 太乙數 — nine star-envoys and a
count — and what they are read for is the weather: 「十精太乙主要功能是占風晴雨
露」, a casebook of wind, clearing, rain and dew put into the form of a 式. The
first of them is the 天皇太乙 of 卷七 this file already cites for its 小周 of
twenty; the group is not computed and is not drawn, and it is named here
because § "The per-palace readings" below says what it qualifies. Reading it
would want the eight states 旺, 相, 胎, 沒, 死, 囚, 休, 廢 that turn with the
八節 — stated for 立春 alone, and on the 洛書's numbers, which 盧央 stops to
warn about in six characters: 「注意不是太乙九宮」.

**The 五行 of the actors, and the planet each stands for.** 太乙 is 木 and the
歲星, 文昌 火 and the 熒惑, 始擊 土 and the 填星, 主大將 金 and the 太白, 客大將
水 and the 辰星, the two 參將 taking what their general generates, 計神 火. This
engine gives an element to each of the sixteen **seats**, which is what 卷六
states and what the drawing tints a palace by, and none to the **roles** — so
the 生剋 between the bodies on a board, which is what that table exists for, is
not computed. It is one table away and it is not there.

**陽九百六, 太乙入卦 and 九宮分野.** Three more chapters of the received layer:
the two great limits of 4560 and 4320 years, the board read into hexagrams, and
the geography of the palaces. Not computed, and the first is the dynastic
register at its most epochal.
...
- **格 is the guest's alone.** 「客目大小將與太乙對宫為格」: the 客 qualifies
  ...
  made; `test/taiyi.test.ts` now holds it to the guest's three. **A second
  tradition does state it for somebody**, and the reading stands anyway:
  盧央's board for 124 marks 「主大將居九宮，又與太乙相格」, which is 《統宗》's
  reading and not 卷三's, and the same board lets a 沖 of branches make a 格
  where this engine tests the facing palace alone. Following one witness where
  two disagree is what this register does; naming a value for the other would
  be naming a parameter for a recension, which it does not.
...
It is also the answer to a question worth stating plainly, and the answer has
one exception that arrived later: **what 太乙 hands down for a palace is a
province and a political omen, and nothing else.** The 之義 clauses of 卷三 are
...
**The exception is the 十精, and it is not about a palace.** That group is read
for wind and rain rather than for reigns — see § "What is not here" — so the
sentence this file used to carry, that there is no non-dynastic interpretive
layer in this art to extract, was too wide. It holds for the eight palaces,
which is what it was written about. Whether 卷七 carries the whole group or
only the 天皇太乙 it states there has not been asked.
...
**And that absence is now an unasked question rather than a closed one.**
中國古代星占學 has a chapter on exactly this, 太乙式數占 pp. 492–499, of which
only the closing summary of eight principles has been read here — the first two
of which, 「以得算之多少定勝負」 and 「以得算之奇偶定陰陽、孤單」, are the two
accounts' subject. What that chapter settles, if anything, has not been put to
the two. Last checked 2026-09-03.
...
«Nothing in this section depends on it» was true three times over and is not
true now. It described a book nobody owned, then work nobody had done, then
work begun; the work is finished, and one of the entries below — the two grid
cells — is now what a shipped quantity stands on rather than what confirms it.
It was the standing adjudicator for every question this section left open, and
the list was short enough to write down — **struck through as each was
answered, and every one of them now is.**

- ~~The defective 火玲 line of 安身主, which both editions in hand print
  identically and neither can settle.~~ Asked. The block cannot settle it
  either, and it fixes the graph on the way.
- ~~The 庚 line of the 四化, where the two editions read 陰同 against 同陰 and
  the split is carried as a variant rather than resolved.~~ Answered, with a
  third reading.
Three of them stood on two witnesses of one lineage, which is not the same as
two lineages, and the block is a third recension for each:

- ~~火星 and 鈴星 off the hour.~~ The block is silent about the hour too, and
  settles the 丑/午 variant while it is there.
- ~~解神 off the year.~~ Confirmed, character for character.
- ~~壬 giving 科 to 天府.~~ Confirmed.
- ~~The two repaired grid cells, forced by arithmetic and confirmed by a
  transcription.~~ The block prints both whole, and prints the other
  hundred and forty-eight to match.
- ~~The starting age of the 大限, which is nowhere a sentence in either edition
  and is induced from the opening words of the bureau verses.~~ Asked at the
  section where the rule would live, and 安大限訣 is two lines long and carries
  no age. The induction stands, and the block makes the series it is drawn
  from complete for the first time.

A seven-juan recension also carries the 批命 material the three-juan one lacks,
which is the part this project declines and would not read.

**The adjudicator has been opened, and this is what it said.**
...
**火星 and 鈴星 close the same way, and settle a variant on the side.** The
block's 安火鈴二星訣 stands at the head of the same leaf — 「寅午戌人丑卯方，
申子辰人寅戌場，亥卯未人酉戌房，巳酉丑人卯戌位」 — and that is the whole
section: a heading, four half-lines of seats, and the next section's circle.
**丑, not 午**, which closes the variant the second edition opened and makes the
午 that edition's own. And no clause counts on by the hour, in a third recension
of a different lineage. That silence is what the shipped default rests on, and
it is a silence rather than a statement: both readings share the seats and part
only on whether one counts on from them, so a verse that gives seats and stops
is consistent with either. Three recensions silent is a stronger silence than
two, and no more than that. Read on the plate 2026-08-28.
...
**解神 is confirmed, and the block draws the distinction the name hides.** Leaf
157 heads a section 「○安天德月德解神訣」 and puts three stars in it. The first
two are placed 「至**流年**太歲上」; the third reads 「解神從戌上起子逆數至**當生
年**太歲上是也」, which is the shipped line character for character, off the
birth year. So the divergence this register has been calling two quantities
wearing one name is not an inference from two agreeing transcriptions: the
block sets 流年 and 當生年 three lines apart in one section, which is a text
distinguishing them rather than a reader doing it. A third recension, and the
first that is neither a transcription nor descended from one. Read on the plate
2026-08-29.

**The two repaired cells are printed, and the repair stops being one.** The
five 紫微 grids stand at leaves 167 to 171, one to a leaf, in the order the
section is titled for — 定金木水火土局, so 金四, 木三, 水二, 火六, 土五. 木三局's
寅 reads 「初三　初五」 and 金四局's 亥 reads 「初一　三十」: the two cells the
first edition dropped a character and a column from, whole. Both were read at
600 dpi with the cell cropped, because each turns on one character — 五 against
九, and whether 三十 stands inside the 亥 rule or the 子 one.

**And the other hundred and forty-eight agree too.** All five grids were read
off the plate and collated against `ZIWEI_BY_DAY`, cell by cell: 150 cells, no
disagreement. What that retires is not a doubt about the arithmetic — the
arithmetic reproduced 148 cells before this — but the shape of the claim. The
two cells used to stand on a rule the book does not state plus a transcription
that prints them; they now stand on a Ming block that prints them, and the rule
is back to being a check rather than a repair. Read on the plate 2026-08-29.

**The 大限's starting age was asked for at the section that would carry it, and
the section does not carry it.** 安大限訣, leaf 158, is two lines whole:
「陽男陰女從命前一宮起是父母宮／陰男陽女從命後一宮起是兄弟宮」, and then the
block frame closes and 安小限訣 opens on the next leaf. No age, and no room for
one. That is a documented negative on the adjudicator, which is the strongest
form this question can take short of an edition stating the rule.

The same two lines confirm two things on the way. The run opens in the palace
**beside** the 命宮, in a third recension and naming the palace it lands in —
which is the shipped reading and the one `daxian: ming` would depart from, now
with three witnesses under it. And the block drops 順行 and 逆行 exactly as the
second edition does, carrying the direction in 前 and 後 instead; so the
Wikisource recension's 「從命前一宮起**順行**」 is that recension's, and the rule
survives without it.

**Where the age does live, the block completes the series.** Each grid is
headed by a placement verse, and each verse opens on its bureau's number
dressed as an age: 「坎水宮中**二歲行**」, 「生逢木宮**三歲遊**」,
「紫微金宮**四歲花**」, 「戊土**五歲**居其中」, 「離火宮中**六歲奇**」. Both
transcriptions break that series at 土五, reading 五**龍**居其中 — and the
fuller of the two flags its own doubt about the character before it, printing
戌'土'（古文中为'水'）where the block reads a plain 戊土. So until this leaf was
read, four of the five verses opened on an age and the fifth did not, and the
induction rested on a pattern with a hole in it. It now rests on five of five.

It is still an induction, and the distinction is worth keeping sharp. 行, 遊,
花, 居 and 奇 are the verbs of a mnemonic, chosen to scan and to rhyme; the
number in front of them is the bureau's step, which is the same number the
tradition starts the run on. Nothing in the book says the run starts there.
What has changed is that the coincidence is now five-fold and printed, not
four-fold and partly conjectured.

**Three more places where the block corrects the transcriptions, and one of
them checks itself.** They are recorded because a collation that only reports
agreements is not one.

- 土五's sixth line reads 「二十九日**午**上尋」 where both transcriptions read
  五上尋. 午 is a palace and 五 is not — and the grid on the same leaf puts
  廿九 in 午, so the block is corrected by its own table. That is the
  self-check this register asks of a lone reading, on the leaf that carries it.
- 木三's first and last lines read **遊** and **儔** where both transcriptions
  read 起 and 尋. The verse's second line ends 牛; 遊 and 儔 rhyme with it and
  起 and 尋 do not, so the block's pair restores a rhyme the transcriptions
  lose. That is an argument from form and not a witness, and it is worth what
  such an argument is worth.
- 木三's fourth line reads 一辰**孚** where both read 一辰字. Recorded and not
  adjudicated: 字 is the easier graph and 孚 keeps the rhyme, which pulls two
  ways.

None of this moves a computed quantity. The verses are mnemonics for grids the
engine ships as a table, and the engine never reads them. Read on the plate
2026-08-29.

**Reading it was the largest thing outstanding on this board, and the list it
was opened against is now struck through entire.** The estimate was 528 pages
of woodblock with no text layer, to adjudicate perhaps eight lines. It came in
far under that, and why is worth recording: the block reads at a hundred dots
to the inch, its 版心 carries the juan on every leaf, and its 目錄 sits at the
front in section order — so a line is located by margin and heading rather than
by sweeping, and leaf 151 was found in four passes. The last three questions
were located off one 目錄 leaf and three contact sheets, and cost seven plates
to read.

What the seven answers came to is worth stating together, because they did not
come out the same way. Two lines were confirmed unchanged — 解神 and 壬's 科.
Two were answered by a silence the block turns out to share — the hour behind
火鈴, the age behind the 大限. One gained a third reading nobody had, at 庚. One
— the two grid cells — stopped being a repair and became a printed text. And
one, 安身主's defective 火玲 line, was asked and came back defective in the same
place, which is how a text's own damage is told from a copyist's. Confirming,
complicating and failing to settle are all outcomes here, and the adjudicator
produced each of them.

**What is left on this book is what was never asked of it.** The 批命 material
a seven-juan recension carries is the class this project declines, and the
...
**And the Ming 南陽堂 block prints them too, along with the other hundred and
forty-eight.** Its five grids stand at leaves 167 to 171, and all 150 cells
were read off the plate and collated against `ZIWEI_BY_DAY` without a
disagreement — 木三局's 寅 reading 「初三　初五」 and 金四局's 亥 reading
「初一　三十」, each read at 600 dpi with the cell cropped, since each turns on
one character. So the repair is retired as a repair. What the two cells stand
on is no longer an unstated arithmetic plus a modern transcription; it is a
...
**starting age of the 大限 is still nowhere stated as a sentence** — not in
either edition, and not in the Ming 南陽堂 block, whose 安大限訣 is two lines
long and gives the palace the run opens in and nothing else. It is carried only
by the opening words of the bureau verses, and remains induced. What the block
changed is that the series those words make is now complete and printed —
二歲行 · 三歲遊 · 四歲花 · 五歲居 · 六歲奇, where both transcriptions break it at
土五 — so the induction has five instances instead of four and still has no
sentence. The reading is above, in the adjudicator's section.
